...
const fetchCollaboratorStats = async () => {
  return [
    { name: "Stubble purchasing company", count: 50, icon: ShoppingCart, type: "STUBBLE_PURCHASING_COMPANY" },
    { name: "Machine rental", count: 30, icon: Tractor, type: "MACHINE_RENTAL" },
    { name: "Transportation company", count: 40, icon: Truck, type: "TRANSPORTATION_COMPANY" },
    { name: "Agriculture shops", count: 60, icon: Store, type: "AGRICULTURE_SHOPS" },
  ]
}
...
const fetchMonthlyStats = async () => {
  return [
    { name: "Jan", collaborators: 20, farmers: 100 },
    { name: "Feb", collaborators: 30, farmers: 150 },
    { name: "Mar", collaborators: 40, farmers: 200 },
    { name: "Apr", collaborators: 50, farmers: 250 },
    { name: "May", collaborators: 60, farmers: 300 },
    { name: "Jun", collaborators: 70, farmers: 350 },
  ]
}
...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const collaboratorStats = await fetchCollaboratorStats()
      const farmerCountData = await fetchFarmerCount()
      const monthlyStatsData = await fetchMonthlyStats()
      setCollaboratorTypes(collaboratorStats)
      setFarmerCount(farmerCountData)
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        {collaboratorTypes.map((collab, index) => (
          <Link to={`/admin/collaborators?type=${collab.type}`} key={index}>
            <Card className="hover:shadow-lg transition-shadow duration-200">
              <CardHeader>
                <CardTitle>{collab.name}</CardTitle>
                <collab.icon className="h-4 w-4 text-muted-foreground" />
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyStats}>
                ...
                <Tooltip />
                <Legend />
                <Line yAxisId="left" type="monotone" dataKey="collaborators" stroke="#8884d8" activeDot={{ r: 8 }} />
                <Line yAxisId="right" type="monotone" dataKey="farmers" stroke="#82ca9d" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
